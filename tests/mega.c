...
char * replaceFunc(char * expr) {
	...
	char * funcName = NULL;
	char ** rawParams = (char **)malloc(10 * sizeof(char * ));
	char ** solvedParams = (char **)malloc(10 * sizeof(char * ));
	int currentParam = 0;
		...
		if (startFunc == -1 && isalpha(expr[i]) )  {
			...
			if (nrParens == 0) {
				...
				for (int j = 0; j < currentParam; ++j) {
					...
					char * tmpC = strdup(curRawParam);

					float tmp = evalExpr(tmpC);
...
int main(int argc, char *argv[])
{
	for (int var = 0; var < 1000; ++var) {
		...
		char *expr = "(3+2)*a(10,b(b(10)),56)+b(99)+c()/2";
